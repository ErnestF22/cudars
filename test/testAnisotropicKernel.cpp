...
int main(int argc, char** argv) {
    cudars::ArsKernelAnisotropic2d ak;
    cudars::ArsKernelIsotropic2d ik;
    cudars::Vector2 mean1, mean2, mean12, v;
    cudars::Matrix2 covar1, covar2, covar12;
    ...
    for (int it = 0; it <= plotStep; ++it) {
        ...
        kernelValFourier[it] = cudars::evaluateFourier(fourierCoeffsAnisot, 2.0 * t);
    ...
    cudars::updateARSF2CoeffRecursDown(lambdaSqNorm, phi, 1.0, arsOrder, fourierCoeffsIsotRaw);
    ...
    ik.setComputeMode(cudars::ArsKernelIsotropic2d::ComputeMode::PNEBI_DOWNWARD);
    ...
    for (int it = 0; it <= plotStep; ++it) {
        t = M_PI * it / plotStep;
        gp << (180.0 / M_PI * t) << " " << normalizer * cudars::evaluateFourier(fourierCoeffsIsotRaw, 2.0 * t) + 2.0 * plotEps << "\n";
    }
    gp << "e\n";
    for (int it = 0; it <= plotStep; ++it) {
        t = M_PI * it / plotStep;
        gp << (180.0 / M_PI * t) << " " << cudars::evaluateFourier(fourierCoeffsAnisot, 2.0 * t) + 3.0 * plotEps << "\n";
    }
    gp << "e\n";
