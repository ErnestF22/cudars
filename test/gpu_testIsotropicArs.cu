...
void rangeToPoint(double* ranges, int num, double angleMin, double angleRes, std::vector<cudars::Vec2d>& points);
...
int main(void) {
    ...
    const int gridTotalSize = cudars::sumNaturalsUpToN(numPts - 1); //total number of threads in grid Fourier coefficients grid - BEFORE PADDING
    ...
    //conversion
    std::vector<cudars::Vec2d> acesPointsSTL;
    //    cudars::Vec2d p0, p1;
    //    p0.x = 0.0;
    //    p0.y = 0.0;
    ...
    thrust::host_vector<cudars::Vec2d> acesPointsHost(acesPointsSTL.begin(), acesPointsSTL.end());


    //    cudars::Vec2d firstElement; //??
    ...
    cudaMalloc((void**) &kernelInput1, numPtsAfterPadding * sizeof (cudars::Vec2d));
    cudaMemcpy(kernelInput1, acesPointsHost.data(), numPtsAfterPadding * sizeof (cudars::Vec2d), cudaMemcpyHostToDevice);
    ...
    ars2.setComputeMode(cudars::ArsKernelIso2dComputeMode::PNEBI_LUT);

    cudars::PnebiLUT pnebiLUT2; //LUT setup
    ...
    cudaMalloc((void **) &kernelInput2, numPtsAfterPadding * sizeof (cudars::Vec2d));
    pnebiMode = cudars::ArsKernelIso2dComputeMode::PNEBI_LUT;
    ...
    int bbnum = 32;
    std::vector<cudars::BoundInterval> bbbs(bbnum);
    for (int i = 0; i < bbnum; ++i) {
        bbbs[i].x0 = M_PI * i / bbnum;
        bbbs[i].x1 = M_PI * (i + 1) / bbnum;
        cudars::findLUFourier(ars1.coefficients(), bbbs[i].x0, bbbs[i].x1, bbbs[i].y0, bbbs[i].y1);
        std::cout << i << ": x0 " << RAD2DEG(bbbs[i].x0) << " x1 " << RAD2DEG(bbbs[i].x1) << ", y0 " << bbbs[i].y0 << " y1 " << bbbs[i].y1 << std::endl;
    }


    cudars::FourierOptimizerBB1D optim(ars1.coefficients());
    ...
    double xopt2, ymax2;
    cudars::findGlobalMaxBBFourier(ars1.coefficients(), 0, M_PI, M_PI / 180.0 * 0.5, 1.0, xopt2, ymax2);
